...
if [[ "$skip_prebuild" -eq 0 ]]; then
  ...
  backend_executable="$repo_root/dist/Echo/Echo"
  ...
  for executable in \
    "$repo_root/dist/Echo/_internal/runtime/base/bin/python" \
    "$repo_root/dist/Echo/_internal/runtime/base/bin/python3" \
    "$repo_root/dist/Echo/_internal/bin/ffmpeg" \
    "$repo_root/dist/Echo/_internal/bin/ffprobe"; do
