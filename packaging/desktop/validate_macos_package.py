...
def validate_app_bundle(app_path: Path, *, version: str, arch: str, bundle_id: str, owner: str, repo: str) -> None:
    ...
    backend_dir = require_dir(resources_dir / "backend" / "Echo", "packaged backend directory")
...
def validate_dmg(dmg_path: Path, *, version: str, arch: str, bundle_id: str, owner: str, repo: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="echo-dmg-") as mount_dir_raw:
...
def validate_zip(zip_path: Path, *, version: str, arch: str, bundle_id: str, owner: str, repo: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="echo-zip-") as extract_dir_raw:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate Echo macOS DMG and ZIP packages.")
    parser.add_argument("--dmg", required=True, type=Path)
    parser.add_argument("--zip", required=True, type=Path)
    parser.add_argument("--version", required=True)
    parser.add_argument("--arch", required=True, choices=("x64", "arm64"))
    parser.add_argument("--bundle-id", default="com.echo.desktop")
    parser.add_argument("--update-owner", default="lycohana")
    parser.add_argument("--update-repo", default="Echo")
