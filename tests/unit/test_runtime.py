...
def test_probe_script_does_not_require_add_dll_directory_on_non_windows(monkeypatch) -> None:
    from video_sum_service import probe_script

    fake_torch = types.SimpleNamespace(
        __file__="/tmp/site-packages/torch/__init__.py",
    ...
    monkeypatch.setattr(os.path, "isdir", lambda path: str(path).endswith("torchaudio/lib"))

    payload = probe_script.probe()

    assert payload["torchInstalled"] is True
    assert "localAsrInstalled" in payload
