...
def test_visual_llm_connection_sends_image_and_uses_visual_config(monkeypatch, tmp_path: Path) -> None:
    ...
    class FakeResponse:
        status_code = 200
        text = '{"choices":[{"message":{"content":"{\\"ok\\":true,\\"text\\":\\"ECHO\\",\\"shape\\":\\"pink circle\\"}"}}]}'

        def json(self) -> dict[str, object]:
            return {"choices": [{"message": {"content": '{"ok":true,"text":"ECHO","shape":"pink circle"}'}}]}
...
def test_asr_connection_uses_unsaved_payload(monkeypatch, tmp_path: Path) -> None:
    ...
    assert file_name == "echo-asr-test-zh.wav"
