...
> 说明：本文件保留为浅色模式视觉提案。当前开发与实现请优先参考 `docs/echo-ui-spec.md`。若与当前代码不一致，以 `apps/desktop/src/app.css` 和主规范为准，尤其是深色模式与平台标签实现。

## 1. 白天模式重设计说明

- 设计方向：明亮、轻专业、带一点消费级产品的精致感，避免传统后台的厚重感。
- 品牌气质：从 Echo logo 提取 `#FB7299 / #F85D8E / #FF9ABA` 作为暖粉珊瑚品牌主轴，用低饱和浅底承接。
- 产品属性表达：
...
- 容器 `42px × 24px`，胶囊形 `border-radius: 24px`
- 关闭态：浅灰 `#e0e0e0`，滑块白色 `18px × 18px` 圆，带微阴影
- 开启态：品牌粉 `#fb7299`，滑块右移 `18px`
- 禁用态：整体 40% 透明度，光标变 `not-allowed`
- 每行左侧标题 + 说明，右侧开关；暗色主题下品牌色、滑块色不变，边框色适配
- 完整 CSS 见 `echo-ui-spec.md §5.9.1`，两份文档保持同步
