...
_install_log_dir = Path(tempfile.gettempdir()) / "echo_install_logs"
...
def _ensure_runtime_sitecustomize(runtime_channel: str) -> None:
    ...
    content = '''"""Auto-generated by Echo — do not edit.
...
if _bs_orig is not None:
    ...
    _bs_os.environ["ECHO_SITECUSTOMIZE"] = "1"
'''
...
def _sanitized_host_env() -> dict[str, str]:
    """Host os.environ with venv-launcher vars stripped and UTF-8 forced, for child pip subprocesses."""
    env = dict(os.environ)
    for key in ("PYTHONHOME", "PYTHONPATH", "PYTHONEXECUTABLE", "__PYVENV_LAUNCHER__"):
        env.pop(key, None)
    env["PYTHONIOENCODING"] = "utf-8"
...
def _make_install_pip_runner(session_id: str | None, label: str, *, use_current_python: bool, runner):
    """Pip runner for an install: a streaming runner when a session is active (so logs stream), else the plain runner."""
    if session_id is None:
        return runner
    start_install_session(session_id, label)
    if use_current_python:
        return _StreamingRunner(session_id, env=_sanitized_host_env(), cwd=repo_root())
    return _StreamingRunner(session_id)


def _abort_streaming_install(pip_runner, session_id: str | None, runtime_channel: str, lock) -> None:
    """Roll back a failed install: cancel the streaming session, clear the probe cache, release the channel lock."""
...
def install_local_asr(reinstall: bool, repository: SqliteTaskRepository, *, session_id: str | None = None) -> tuple[dict[str, object], TaskWorker]:
    ...
    use_streaming = session_id is not None
    pip_runner = _make_install_pip_runner(session_id, "本地 ASR", use_current_python=use_current_python, runner=runner)
    ...
    try:
        ...
        _abort_streaming_install(pip_runner, session_id, runtime_channel, lock)
...
def install_funasr(reinstall: bool, repository: SqliteTaskRepository, *, session_id: str | None = None) -> tuple[dict[str, object], TaskWorker]:
    ...
    use_streaming = session_id is not None
    pip_runner = _make_install_pip_runner(session_id, "FunASR", use_current_python=use_current_python, runner=runner)
