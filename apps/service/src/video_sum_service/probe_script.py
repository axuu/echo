...
def probe() -> dict:
    ...
    if torch is not None:
        ...
        add_dll_directory = getattr(sys.modules["os"], "add_dll_directory", None)
        if add_dll_directory is not None and sys.modules["os"].path.isdir(_tal):
            add_dll_directory(_tal)
    ...
    payload["sitecustomizeActive"] = sys.modules["os"].environ.get("ECHO_SITECUSTOMIZE") == "1"
