...
# visual_note_status starts at "idle" and may be "skipped" when the pipeline opts out;
# both mean no note has been produced yet, so auto-generation may proceed.
_VISUAL_NOTE_PENDING_STATUSES = frozenset({"idle", "skipped"})
# A note that is already generating or ready must not be auto-triggered again.
_VISUAL_NOTE_IN_FLIGHT_STATUSES = frozenset({"generating", "ready"})
...
class TaskWorker:
    ...
    def _run_task(self, task_id: str) -> None:
        ...
        try:
            ...
            if (
                ...
                and final_result.visual_note_status in _VISUAL_NOTE_PENDING_STATUSES
    ...
    def _can_generate_visual_evidence(self, record: TaskRecord, result: TaskResult) -> bool:
        if result.visual_note_status in _VISUAL_NOTE_IN_FLIGHT_STATUSES:
            return False
        if not hasattr(self._pipeline_runner, "build_and_export_visual_evidence"):
            return False
        if self._auto_visual_note_mode(record) == "text":
            return False
        return self._has_visual_note_prerequisites(result)

    def _auto_visual_note_mode(self, record: TaskRecord) -> str:
        """Mode that auto-generation honors: an explicit task option (default text), else the settings default."""
        task_mode = getattr(record.task_input.options, "visual_note_mode", None)
        if task_mode is not None:
            return normalize_visual_note_mode(task_mode)
        settings = getattr(self._pipeline_runner, "_settings", None)
        return normalize_visual_note_mode(getattr(settings, "visual_note_mode", "frame_insert"), default="frame_insert")

    def _has_visual_note_prerequisites(self, result: TaskResult) -> bool:
        return bool(
            result.artifacts.get("summary_path") and result.timeline and result.knowledge_note_markdown.strip()
        )
    ...
    def _load_subjob_record(self, task_id: str, *, capability: str, label: str) -> TaskRecord | None:
        """Load a completed task record for a follow-up sub-job, or return None (logged) if it cannot run."""
        record = self._repository.get_task(task_id)
        if record is None or record.result is None:
            logger.warning("skip missing task result for %s task_id=%s", label, task_id)
            return None
        if record.status != TaskStatus.COMPLETED:
            logger.warning("skip non-completed task %s task_id=%s status=%s", label, task_id, record.status.value)
            return None
        if not hasattr(self._pipeline_runner, capability):
            logger.warning("pipeline runner does not support %s task_id=%s", label, task_id)
            return None
        return record
    ...
    def _begin_subjob(
        ...
        *,
        ...
        """Flip a sub-job's status to "generating", persist it, and emit the start event."""
        current_result = result.model_copy(update={status_field: "generating", error_field: None})
        self._repository.save_result(task_id, current_result)
        self._repository.append_event(task_id=task_id, stage=stage, progress=progress, message=message, payload=payload)
    ...
    def _fail_subjob(
        ...
        *,
        ...
        """Persist a sub-job failure on the latest result and emit the failure event."""
        refreshed = self._repository.get_task(task_id)
        if refreshed is None or refreshed.result is None:
            return
        update: dict[str, object] = {status_field: "failed", error_field: str(exc)}
        if updated_field is not None:
            update[updated_field] = datetime.now(timezone.utc)
        failed_result = refreshed.result.model_copy(update=update)
        self._repository.save_result(task_id, failed_result)
        self._repository.append_event(task_id=task_id, stage=stage, progress=100, message=message, payload=payload)
    ...
    def _run_visual_evidence(self, task_id: str, force: bool, mode: str | None = None) -> None:
        record = self._load_subjob_record(
            task_id, capability="build_and_export_visual_evidence", label="visual evidence"
        )
        if record is None:
            return
        ...
        current_result = self._begin_subjob(
            task_id,
            record.result,
            status_field="visual_note_status",
            error_field="visual_note_error_message",
        ...
        try:
            ...
            self._fail_subjob(
                ...
                updated_field="visual_note_updated_at",
            )
    ...
    def _run_mindmap(self, task_id: str, force: bool) -> None:
        record = self._load_subjob_record(
            task_id, capability="build_and_export_mindmap", label="mindmap"
        )
        if record is None:
            return
        ...
        current_result = self._begin_subjob(
            ...
            error_field="mindmap_error_message",
