...
def load_builtin_asr_test_audio() -> tuple[str, bytes, str]:
    ...
    return "echo-asr-test-zh.wav", audio.read_bytes(), "audio/wav"
...
def load_asr_test_audio() -> tuple[str, bytes, str]:
    ...
    try:
        ...
        return "echo-asr-test.wav", build_test_wav_bytes(), "audio/wav"
...
def probe_llm_connection(payload: SettingsUpdatePayload | None = None) -> dict[str, object]:
                    ...
                    "content": [
                        {
                            ...
                            "text": (
                                ...
                                '格式：{"ok":true,"text":"ECHO","shape":"pink circle"}'
                            ),
                        },
                        {
                            "type": "image_url",
    ...
    if test_scope == "visual":
        ...
        if "echo" not in parsed_text and "bili" not in parsed_text:
            raise HTTPException(
                status_code=502,
                detail="视觉模型测试失败：接口返回了 JSON，但未识别出测试图片中的 Echo 文本。",
            )
