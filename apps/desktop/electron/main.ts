...
const rendererUrl = process.env.ECHO_RENDERER_URL ?? process.env.BRIEFVID_RENDERER_URL ?? "http://127.0.0.1:5173";
...
async function exportBilibiliCookies(cookies: Electron.Cookie[]): Promise<BilibiliCookieExportResult> {
  ...
  const lines = [
    ...
    "# Generated by Echo from the in-app Bilibili login window.",
    ...exportableCookies
...
function getSplashMarkup(message = "正在启动 Echo 服务...") {
...
function loadSplash(message = "正在启动 Echo 服务...") {
...
function updateBackendStatus(patch: Partial<BackendStatus>) {
  ...
  if (!previousReady && backendStatus.ready) {
    ...
    updateSplashMessage("Echo 服务已停止，正在等待重新启动。");
  }
}
...
function resolveDevPython(): { command: string; args: string[]; cwd: string; forceHidden?: boolean } {
  ...
  const devPython = process.env.ECHO_DEV_PYTHON ?? process.env.BRIEFVID_DEV_PYTHON;
  if (devPython) {
    return {
      command: devPython,
...
function resolvePackagedBackend(): { command: string; args: string[]; cwd: string } {
  const executableName = process.platform === "win32" ? "Echo.exe" : "Echo";
  const candidateRoots = [
    path.join(process.resourcesPath, "backend", "Echo"),
    path.join(path.dirname(process.execPath), "resources", "backend", "Echo"),
    path.join(path.dirname(app.getAppPath()), "backend", "Echo"),
  ];
  ...
  throw new Error(
    [
      ...
      `请确认安装目录下存在 ${path.join("resources", "backend", "Echo", executableName)}。`,
...
function sendBackendStartingResponse(response: ServerResponse) {
  ...
  response.end(JSON.stringify({ detail: "Echo 后端正在启动，请稍后重试。" }));
}
...
async function startBackend(): Promise<BackendStatus> {
  ...
  if (isDev) {
    ...
    const noBackend = process.env.ECHO_NO_BACKEND === "1";
    if (noBackend) {
      console.log("[Backend] ECHO_NO_BACKEND=1, skipping backend auto-start.");
      updateBackendStatus({
        running: false,
        ready: false,
        pid: null,
  ...
  if (!isDev && await probeBackendPortBusy(300)) {
    const message = "Echo 服务端口已被占用，请退出旧版 Echo 后重试。";
    updateBackendStatus({
      running: false,
      ready: false,
      pid: null,
  ...
  backendProcess.once("exit", (_code, signal) => {
    ...
    if (!forceQuit && !isDev) {
      loadSplash("Echo 服务已停止，正在等待重新启动。");
    }
  });
...
function rebuildTrayMenu() {
  ...
  tray.setToolTip(`Echo ${app.getVersion()} · ${backendStatus.running ? "运行中" : "已停止"}`);
...
async function installAndRestart(): Promise<void> {
  ...
  try {
    if (backendProcess) {
      ...
      if (portStillBusy) {
        throw new Error("后端端口仍被占用，无法安全安装更新。请退出旧版 Echo 后重试。");
      }
    }
    setTimeout(() => {
...
function registerIpcHandlers() {
  ...
  ipcMain.handle("desktop:logs:export", async () => {
    const result = await dialog.showSaveDialog(mainWindow!, {
      title: "导出日志",
      defaultPath: `echo-${new Date().toISOString().slice(0, 10)}.log`,
