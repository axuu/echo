import { type Dispatch, type FocusEvent, type FormEvent, type SetStateAction, useCallback, useEffect, useLayoutEffect, useRef, useState } from "react";
...
import { formatDateTime, formatStorageCount, formatStorageSize, loadHiddenPromptPresetIds, parseMinOneInt, persistHiddenPromptPresetIds, taskStatusLabel } from "../utils";
import { settingsCategories, type SettingsCategory } from "./settingsConfig";
import { VideoSection } from "./settings/VideoSection";
...
// Derives the status pill (label/class) and summary text for a model from its availability probe,
// readiness and enabled flags — shared by the main LLM and visual model sections.
function deriveModelStatus(
  availability: { status: string },
  ready: boolean,
  enabled: boolean,
  labels: { pending: string; summaryPending: string },
      ...
      : availability.status === "checking"
        ...
        : ready
          ...
          : enabled
            ? labels.pending
            : "关闭";
  const className = availability.status === "available" || (availability.status === "unknown" && ready)
    ? "success"
    : availability.status === "unavailable"
      ? "danger"
      : availability.status === "checking" || enabled
        ? "warning"
        : "";
  const summary = enabled
    ...
    : "未启用";
  return { label, className, summary };
}

// Shared "install + live log" lifecycle: streams the install log into setOutput while the install
// runs, guarantees the poll timer is cleared on every exit path, and routes the result/error to
// the caller. Keeps timer cleanup in one place instead of repeating it across each install button.
async function runInstallWithLog<R>(options: {
  sessionId: string;
  setInstalling: (value: boolean) => void;
  setOutput: Dispatch<SetStateAction<string>>;
  install: (sessionId: string) => Promise<R>;
  onComplete: (response: R) => Promise<void>;
  onError: (error: unknown) => void;
  onPollError?: (setOutput: Dispatch<SetStateAction<string>>, error: unknown) => void;
}): Promise<void> {
  ...
  try {
    options.setInstalling(true);
    options.setOutput("");
    pollTimer = setInterval(async () => {
      try {
        const log = await api.getInstallLog(options.sessionId);
        if (log.log) options.setOutput(log.log);
        if (log.done) stopPolling();
      } catch (error) {
        options.onPollError?.(options.setOutput, error);
      }
    }, 1500);
    const response = await options.install(options.sessionId);
    stopPolling();
    try {
      const log = await api.getInstallLog(options.sessionId);
      if (log.log) options.setOutput(log.log);
...
export function SettingsPage({
  ...
  const { label: mainModelStatusLabel, className: mainModelStatusClass, summary: mainModelSummary } =
    deriveModelStatus(mainModelAvailability, llmReady, llmEnabled, { pending: "待配置", summaryPending: "待补全" });
  const { label: visualModelStatusLabel, className: visualModelStatusClass, summary: visualModelSummary } =
    deriveModelStatus(visualModelAvailability, visualLlmReady, visualMultimodalEnabled, { pending: "待确认", summaryPending: "跟随或待补全" });
  ...
  async function applyTranscriptionInstallResult(
    response: { installed: boolean; environment?: EnvironmentInfo },
    currentForm: ServiceSettings,
    options: { provider: string; switchedStatus: string; setStatus: (value: string) => void; partialFailPrefix: string },
  ) {
    const nextEnvironment = response.environment || (await api.getEnvironment({ runtimeChannel: currentForm.runtime_channel, refresh: true }));
    setEnvironment(nextEnvironment);
    if (response.installed) {
      try {
        const settingsResponse = await api.updateSettings({
          ...buildSettingsSavePayload(currentForm),
          transcription_provider: options.provider,
        });
        setForm(maskConfiguredApiKeys(settingsResponse.settings));
        setIsDirty(false);
        setSaveStatus(settingsResponse.message || options.switchedStatus);
        onSettingsSaved(settingsResponse.settings, nextEnvironment);
      } catch (error) {
        options.setStatus(`${options.partialFailPrefix}：${error instanceof Error ? error.message : "保存设置失败"}`);
      }
    }
    onRefresh();
  }
  ...
  async function installLocalAsr() {
    ...
    await runInstallWithLog({
      sessionId: `local-asr-${Date.now()}`,
      setInstalling: setLocalAsrInstalling,
      setOutput: setLocalAsrOutput,
      install: (sessionId) => api.installLocalAsr({ installSessionId: sessionId }),
      onComplete: async (response) => {
        setLocalAsrStatus(response.installed ? "本地语音识别环境已安装" : "本地语音识别环境安装失败");
        await applyTranscriptionInstallResult(response, currentForm, {
          provider: "local",
          switchedStatus: "已切换为本地 ASR",
          setStatus: setLocalAsrStatus,
          partialFailPrefix: "本地 ASR 已安装，但切换默认转写方式失败",
        });
      },
      onError: (error) => setLocalAsrStatus(error instanceof Error ? error.message : "安装本地语音识别环境失败"),
    });
  }
  ...
  async function installFunAsr() {
    ...
    await runInstallWithLog({
      sessionId: `funasr-${Date.now()}`,
      setInstalling: setFunasrInstalling,
      setOutput: setFunasrOutput,
      install: (sessionId) => api.installFunAsr({ installSessionId: sessionId }),
      onPollError: (setOutput, error) =>
        setOutput((prev) => prev + `\n[轮询异常: ${error instanceof Error ? error.message : "network error"}]`),
      onComplete: async (response) => {
        setFunasrStatus(response.installed ? "FunASR 语音识别环境已安装" : "FunASR 语音识别环境安装失败");
        await applyTranscriptionInstallResult(response, currentForm, {
          provider: "funasr",
          switchedStatus: "已切换为 FunASR",
          setStatus: setFunasrStatus,
          partialFailPrefix: "FunASR 已安装，但切换默认转写方式失败",
        });
      },
      onError: (error) => setFunasrStatus(error instanceof Error ? error.message : "安装 FunASR 语音识别环境失败"),
    });
  }
        ...
        <div className="settings-content-scroll" ref={settingsContentScrollRef}>
          ...
          {activeCategory === "files" && (
            <FilesSection
              ...
              cleanupTargetCount={cleanupTargetCount}
              refreshStorageOverview={refreshStorageOverview}
              openManagedDirectory={openManagedDirectory}
              cleanupManagedFiles={cleanupManagedFiles}
            />
          )}
          ...
          {activeCategory === "video" && (
            <VideoSection
              ...
              bilibiliCookieCapturing={bilibiliCookieCapturing}
              bilibiliCookieStatus={bilibiliCookieStatus}
              bilibiliQrcodeImage={bilibiliQrcodeImage}
              captureBilibiliLoginCookies={captureBilibiliLoginCookies}
            />
          )}
              ...
              <div className="settings-form-group">
                <label className="settings-input-group">
                  ...
                  <select
                    ...
                    ref={registerFocusTarget("transcription_provider")}
                ...
                <div className="settings-input-group">
                  <div className="settings-toggle-row">
                    <div className="settings-toggle-label">
                      ...
                      <span className="settings-toggle-caption">开启后，Echo 会尝试从 B 站 API 获取视频自带字幕（CC 字幕、UP 主上传字幕）。如果字幕存在且完整，将跳过 ASR 转写，节省时间。字幕不存在时自动使用 ASR。</span>
                    </div>
                ...
                {usesSiliconFlowAsr ? (
                  <>
                    <label
                      ...
                      ref={registerFocusTarget("siliconflow_asr_base_url")}
                    >
                    ...
                    <label
                      ...
                      ref={registerFocusTarget("siliconflow_asr_api_key")}
                    >
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("siliconflow_asr_model")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("siliconflow_asr_chunk_duration_seconds")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("siliconflow_asr_concurrency")}>
                  ...
                  <>
                    <label
                      ...
                      ref={registerFocusTarget("multimodal_asr_base_url")}
                    >
                    ...
                    <label
                      ...
                      ref={registerFocusTarget("multimodal_asr_api_key")}
                    >
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("multimodal_asr_model")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("multimodal_asr_chunk_duration_seconds")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("multimodal_asr_max_retries")}>
                  ...
                  <>
                    <label className="settings-input-group" ref={registerFocusTarget("funasr_model")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("funasr_device")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("funasr_vad_model")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("funasr_punc_model")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("funasr_spk_model")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("funasr_hub")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("funasr_hotword")}>
                  ...
                  <>
                    <label className="settings-input-group" ref={registerFocusTarget("device_preference")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("fixed_model")}>
            ...
            <section className="settings-category-section generation-settings-section">
              ...
              <div className="generation-settings-tree">
                <section className="settings-tree-panel">
                  ...
                  <div className="settings-tree-grid">
                    <label className="settings-input-group">
                      ...
                      <select
                        className="settings-select-field"
                        ref={registerFocusTarget("llm_enabled")}
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("summary_mode")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("language")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("summary_chunk_retry_count")}>
                ...
                <section className="settings-tree-panel">
                  ...
                  <div className="settings-model-summary-grid">
                    ...
                    <article
                      ...
                      ref={registerFocusTarget("visual_multimodal_enabled")}
                    >
                      <div className="settings-model-card-top">
                        <div>
                ...
                <section className="settings-tree-panel">
                  ...
                  <div className="settings-tree-grid">
                    <label className="settings-input-group" ref={registerFocusTarget("auto_generate_mindmap")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("prompt_router_mode")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("visual_note_mode")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("visual_evidence_enabled")}>
                ...
                {(form.visual_note_mode || "text") !== "text" ? (
                  <section className="settings-tree-panel">
                    ...
                    <div className="settings-tree-grid">
                      <label className="settings-input-group" ref={registerFocusTarget("visual_download_resolution")}>
                ...
                <section className="settings-tree-panel">
                  ...
                  <div className="settings-tree-grid">
                    <label className="settings-input-group" ref={registerFocusTarget("summary_chunk_target_chars")}>
                    ...
                    <label className="settings-input-group" ref={registerFocusTarget("summary_chunk_overlap_segments")}>
            ...
            <section className="settings-category-section generation-settings-section">
              ...
              <div className="generation-settings-tree">
                <section className="settings-tree-panel">
                  ...
                  <div className="settings-tree-grid">
                    <label className="settings-input-group" ref={registerFocusTarget("knowledge_enabled")}>
                ...
                <section className="settings-tree-panel">
                  ...
                  <div className="settings-tree-grid">
                    <div
                      ...
                      ref={registerFocusTarget("knowledge_dependencies_alert")}
                    >
                    ...
                    <label className="settings-input-group">
                      ...
                      <div
                        ...
                        ref={registerFocusTarget("knowledge_dependencies")}
                      >
                ...
                <section className="settings-tree-panel">
                  ...
                  <div className="settings-tree-grid">
                    <label
                      ...
                      ref={registerFocusTarget("knowledge_embedding_provider")}
                    >
                    ...
                    {form.knowledge_embedding_provider !== "online" ? (
                      <>
                        ...
                        <label
                          ...
                          ref={registerFocusTarget("knowledge_embedding_model")}
                        >
                        ...
                        {form.knowledge_embedding_provider === "local_huggingface" ? (
                          <label
                            ...
                            ref={registerFocusTarget("hf_endpoint")}
                          >
                ...
                <section className="settings-tree-panel">
                  ...
                  <div className="settings-tree-grid">
                    <label
                      ...
                      ref={registerFocusTarget("knowledge_llm_mode")}
                    >
                    ...
                    {knowledgeLlmUsesCustom ? (
                      <>
                        <label
                          ...
                          ref={registerFocusTarget("knowledge_llm_enabled")}
                        >
                        ...
                        {form.knowledge_llm_enabled ? (
                          <>
                            <label className="settings-input-group" ref={registerFocusTarget("knowledge_llm_provider")}>
                            ...
                            <label className="settings-input-group" ref={registerFocusTarget("knowledge_llm_base_url")}>
                            ...
                            <label className="settings-input-group" ref={registerFocusTarget("knowledge_llm_api_key")}>
                            ...
                            <label className="settings-input-group" ref={registerFocusTarget("knowledge_llm_model")}>
                              <span className="settings-input-label">模型名称</span>
            ...
            <section className="settings-category-section settings-prompts-section">
              ...
              <div className="settings-prompt-list">
                <details className="settings-prompt-collapse" ref={(node) => { promptDetailsRefs.current.summary = node; }} onToggle={(e) => handleOuterToggle("summary", e)}>
                  ...
                  <div className="settings-form-group">
                  <label className="settings-input-group" ref={registerFocusTarget("summary_system_prompt")}>
                ...
                <div className={`settings-prompt-collapse settings-focus-target ${presetsSectionOpen ? " open" : ""}`} ref={registerFocusTarget("prompt_presets_library")}>
                  <div
                ...
                <details className="settings-prompt-collapse" ref={(node) => { promptDetailsRefs.current.knowledge = node; }} onToggle={(e) => handleOuterToggle("knowledge", e)}>
                  ...
                  <div className="settings-form-group">
                  <label className="settings-input-group" ref={registerFocusTarget("knowledge_note_system_prompt")}>
                ...
                <details className="settings-prompt-collapse" ref={(node) => { promptDetailsRefs.current.visual = node; }} onToggle={(e) => handleOuterToggle("visual", e)}>
                  ...
                  <div className="settings-form-group">
                  <label className="settings-input-group" ref={registerFocusTarget("visual_note_system_prompt")}>
                ...
                <label className="settings-input-group" ref={registerFocusTarget("visual_vlm_prompt")}>
                ...
                <label className="settings-input-group" ref={registerFocusTarget("visual_note_user_prompt_template")}>
          ...
          {activeCategory === "performance" && (
            <PerformanceSection
              ...
              updateForm={updateForm}
              registerFocusTarget={registerFocusTarget}
              recommendedTaskConcurrency={recommendedTaskConcurrency}
              performanceRecommendation={performanceRecommendation}
              setTaskListOpen={setTaskListOpen}
            />
          )}
              ...
              <div className="settings-form-group">
                <div className="settings-input-group">
                  ...
                  <div
                    ...
                    ref={registerFocusTarget("local_asr_runtime")}
                  >
              ...
              <div className="settings-form-group">
                <div className="settings-input-group">
                  ...
                  <div
                    ...
                    ref={registerFocusTarget("funasr_runtime")}
                  >
          ...
          {activeCategory === "logs" && (
            <LogsSection
              ...
              setSilentStartEnabled={setSilentStartEnabled}
              crashAutoRestart={crashAutoRestart}
              setCrashAutoRestart={setCrashAutoRestart}
              ...
              filteredLogOutput={filteredLogOutput}
              logTextareaRef={logTextareaRef}
