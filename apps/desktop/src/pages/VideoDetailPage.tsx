...
import { useEffect, useMemo, useRef, useState, type CSSProperties, type Dispatch, type PointerEvent as ReactPointerEvent, type SetStateAction, type SVGProps } from "react";
...
const SUMMARY_PREFERENCE_STORAGE_KEY = "echo.summaryPreference";
...
// Shared loading/error/data scaffold for a per-task resource fetch: flips the loading flag,
// stores the response (or a caller-built failed state) keyed by taskId, and always clears loading.
async function fetchTaskResource<T>(
  taskId: string,
  call: () => Promise<T>,
  buildFailed: (message: string) => T,
  fallbackMessage: string,
  setData: Dispatch<SetStateAction<Record<string, T>>>,
  setLoading: Dispatch<SetStateAction<Record<string, boolean>>>,
): Promise<T> {
  setLoading((current) => ({ ...current, [taskId]: true }));
  try {
    const response = await call();
    setData((current) => ({ ...current, [taskId]: response }));
    return response;
  } catch (error) {
    const failed = buildFailed(error instanceof Error ? error.message : fallbackMessage);
    setData((current) => ({ ...current, [taskId]: failed }));
    throw error;
  } finally {
    setLoading((current) => omitRecordKey(current, taskId));
  }
}
...
export function VideoDetailPage({ refreshToken = 0, onRefresh, onOpenCookieSettings, onOpenCookieTutorial, onCaptureLoginCookies }: VideoDetailPageProps) {
  ...
  function buildFailedMindMap(taskId: string, message: string): TaskMindMapResponse {
    return { task_id: taskId, status: "failed", error_message: message, updated_at: null, mindmap: null };
  }

  function buildFailedVisualEvidence(taskId: string, mode: string, message: string): TaskVisualEvidenceResponse {
    return {
      task_id: taskId,
      mode,
      status: "failed",
  ...
  async function loadMindMap(taskId: string, options: { force?: boolean } = {}) {
    ...
    return fetchTaskResource(
      taskId,
      () => api.getTaskMindMap(taskId),
      (message) => buildFailedMindMap(taskId, message),
      "思维导图加载失败",
      setMindMaps,
      setMindMapLoading,
    );
  ...
  async function triggerMindMapGeneration(taskId: string, options: { force?: boolean } = {}) {
    return fetchTaskResource(
      taskId,
      () => api.generateTaskMindMap(taskId, { force: options.force }),
      (message) => buildFailedMindMap(taskId, message),
      "思维导图生成失败",
      setMindMaps,
      setMindMapLoading,
  ...
  async function loadVisualEvidence(taskId: string, options: { force?: boolean } = {}) {
    ...
    return fetchTaskResource(
      taskId,
      () => api.getTaskVisualEvidence(taskId),
      (message) => buildFailedVisualEvidence(taskId, "text", message),
      "图文笔记加载失败",
      setVisualEvidence,
      setVisualEvidenceLoading,
    );
  }
  ...
  async function triggerVisualNoteGeneration(taskId: string, options: { force?: boolean; mode?: string } = {}) {
    return fetchTaskResource(
      taskId,
      () => api.generateTaskVisualEvidence(taskId, { force: options.force, mode: options.mode }),
      (message) => buildFailedVisualEvidence(taskId, "frame_insert", message),
      "图文笔记生成失败",
      setVisualEvidence,
      setVisualEvidenceLoading,
    );
