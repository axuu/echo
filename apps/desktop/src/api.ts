...
export class AuthRequiredError extends Error {
  constructor(message = "需要输入 Echo 访问密钥。") {
    super(message);
...
export const api = {
  ...
  listPromptPresets() {
    return fetchJson<PromptPreset[]>("/api/v1/prompts/presets");
  },
  ...
  deletePromptPreset(presetId: string) {
    return fetchJson<{ deleted: boolean; preset_id: string }>(
      `/api/v1/prompts/presets/${encodeURIComponent(presetId)}`,
      { method: "DELETE" },
    );
  },
  matchPrompt(title: string) {
    return fetchJson<PromptMatchResult>("/api/v1/prompts/match", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
