...
export const HIDDEN_PROMPT_PRESETS_STORAGE_KEY = "echo.hiddenPromptPresetIds";
export const PROMPT_PRESETS_VISIBILITY_EVENT = "echo:prompt-presets-visibility-changed";

export function loadHiddenPromptPresetIds(): Set<string> {
...
export function persistHiddenPromptPresetIds(ids: Set<string>): void {
  if (typeof window === "undefined") {
    return;
  }
  window.localStorage.setItem(HIDDEN_PROMPT_PRESETS_STORAGE_KEY, JSON.stringify(Array.from(ids)));
  window.dispatchEvent(new Event(PROMPT_PRESETS_VISIBILITY_EVENT));
}

export function parseMinOneInt(value: string, fallback: number): number {
...
export function formatStorageCount(value: number, noun: string): string {
