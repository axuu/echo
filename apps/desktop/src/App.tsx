...
export function App() {
  ...
  const runtimeStatusText = useMemo(() => {
    if (snapshot.runtimeInitializing) return "加载中";
    if (runtimeStartupStatus === "error") return "异常";
    if (snapshot.environment?.runtimeReady === false) return "未就绪";
    if (snapshot.environment) return "就绪";
    return snapshot.serviceOnline ? "检测中" : "等待服务启动";
  ...
  function resolveVisualNoteModeFromPreference(): string | null {
    try {
      const rawValue = window.localStorage.getItem("echo.summaryPreference");
  ...
  const pageMeta = location.pathname.startsWith("/settings")
    ? { eyebrow: "设置中心", title: "运行配置、环境检测与日志", description: "围绕本地推理环境、模型配置与桌面端服务控制，统一管理 Echo 的运行能力。" }
    : location.pathname.startsWith("/library")
      ...
      : location.pathname.startsWith("/videos/")
        ...
        : { eyebrow: "Echo Workspace", title: "懒得看视频？一键省流！", description: "把时间留给真正有质量的视频" };
    ...
    <div className="auth-shell">
      <section className="auth-panel">
        ...
        <p className="auth-muted">
          Web 端访问需要 Echo 服务密钥。桌面端会自动注入密钥；Docker 或 npx 部署可通过
          {" "}
