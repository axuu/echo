...
def _is_ollama_openai_base_url(base_url: str) -> bool:
    ...
    def _raise_ydl_error(self, error: Exception) -> NoReturn:
        ...
        if "Could not copy Chrome cookie database" in message:
            raise VideoSumError(
                ...
                "请通过 Echo 的 B 站登录窗口重新捕获登录态，或按教程导出 cookies.txt 并填写 yt-dlp Cookies 文件。"
            ) from error
        ...
        if "HTTP Error 412" in message and "BiliBili" in message:
            raise VideoSumError(
                ...
                "或通过 Echo 的 B 站登录窗口捕获登录态；也可以填写 yt-dlp Cookies 文件 "
    ...
    def _request_llm_json(
        ...
        *,
        ...
        if use_anthropic:
            ...
            if payload.get("enable_thinking") is False and _is_ollama_openai_base_url(base_url):
