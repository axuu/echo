...
const RELEASES_URL = "https://github.com/lycohana/Echo/releases/latest";
const REPO_URL = "https://github.com/lycohana/Echo";
...
function printHelp() {
  ...
  console.log("");
  console.log("Usage:");
  console.log("  npx echo                         Start the local Echo service");
  console.log("  npx echo start [options]          Start the local Echo service");
  console.log("  npx echo doctor                   Check Python/runtime setup");
  console.log("  npx echo --version                Print package version");
  console.log("  npx echo release                  Open the latest GitHub release");
  console.log("");
  console.log("Options:");
...
function parseStartOptions(args) {
  const options = {
    host: process.env.ECHO_HOST || "127.0.0.1",
    port: process.env.ECHO_PORT || DEFAULT_PORT,
    python: process.env.ECHO_PYTHON || "",
    data: process.env.ECHO_DATA || defaultDataDir(),
    env: [],
    reinstall: false,
    open: true,
  };
...
function defaultBaseDir() {
  if (process.env.ECHO_NPX_HOME) {
    return process.env.ECHO_NPX_HOME;
  }
  if (process.platform === "win32") {
    return join(process.env.LOCALAPPDATA || process.env.USERPROFILE || process.cwd(), "Echo", "npx");
  }
  return join(process.env.XDG_DATA_HOME || join(process.env.HOME || process.cwd(), ".local", "share"), "echo", "npx");
}
...
function ensureRuntime() {
  if (!existsSync(join(RUNTIME_ROOT, "apps", "service", "pyproject.toml"))) {
    throw new Error("Echo runtime files are missing from this npm package. Please reinstall echo.");
  }
  if (!existsSync(join(RUNTIME_ROOT, "apps", "web", "static", "index.html"))) {
    throw new Error("Echo web static files are missing from this npm package. Please reinstall echo.");
  }
}
...
function ensureVenv(options) {
  ...
  const marker = join(venvDir(), ".echo-installed");
  ...
  console.log("Preparing Echo Python runtime. This may take a minute on first run...");
...
function startService(args) {
  ...
  console.log(`Starting Echo at ${url}`);
...
function doctor(args) {
  ...
  console.log(`Echo package: ${readVersion()}`);
